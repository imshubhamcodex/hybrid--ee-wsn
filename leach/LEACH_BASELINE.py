...
from leach.utils import distance, measure_metrics
from leach.L5_comm import radio_comm
from leach.config import N_NODES, AREA_SIZE, INIT_ENERGY, ROUNDS, N_HALF, P_OPT
...
def run_leach():
    ...
    alive_nodes_history = []
    num_ch_history = []
    reward_history = []          # LEACH doesn’t use RL, keep dummy 0
    ...
    for rnd in range(1, ROUNDS+1):
        ...
        # Each node becomes CH with prob P_OPT if alive
        chs = []
        for i in range(N_NODES):
            if nodes_energy[i] > 0 and random.random() < P_OPT:
                chs.append(i)
        if len(chs) == 0:
            # fallback: pick one random alive node
            alive_nodes = [i for i in range(N_NODES) if nodes_energy[i] > 0]
            if alive_nodes:
                chs.append(random.choice(alive_nodes))
        ...
        reward_history.append(0.0)  # no RL, so 0
        ...
        if alive_after == 0:
            print(f"[LEACH] All nodes died at round {rnd}")
            break
